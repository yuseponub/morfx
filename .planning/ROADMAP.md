...
- [ ] 41-14-PLAN.md — GAP-41-09 gap-closure: align the CLIENT optimistic media type with the server reclassification for audio-only mp4/mov on IG/FB (browser-safe `isAudioOnlyMp4Bytes(Uint8Array)` + composer override) — IG-02 — **PLANNED 2026-06-06**. Fixes the phantom stuck 'sending' clone: the realtime reconciler (use-messages.ts) matches optimistic↔real by type+caption, but GAP-41-08 reclassifies the server row to type='audio' while the composer's optimistic type stays 'video' (deriveMediaType of video/mp4) → no match → real row appended + optimistic stuck forever. Fix refactors `isAudioOnlyMp4(Buffer)` to delegate to a new browser-safe `isAudioOnlyMp4Bytes(Uint8Array)` and applies the SAME audio-only detection in the composer (gated IG/FB + video/mp4|quicktime, decoding the in-memory base64 700KB prefix via atob) so optimistic type='audio' matches → reconciler swaps in place. Reconciler NOT modified; WhatsApp NOT changed (Regla 6). TDD RED→GREEN; existing server is-audio-only-mp4.test.ts stays green; verify runs `pnpm build`. Regla 5 N/A (client-only, no migration). Wave 1 gap-closure, depends_on [].
