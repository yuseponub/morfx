...
import { MessageSquareIcon, PencilIcon, TrashIcon, UserIcon, LoaderIcon } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { Textarea } from '@/components/ui/textarea'
import { Timeline, TimelineItem, formatRelativeDate } from '@/components/ui/timeline'
import { createOrderNote, updateOrderNote, deleteOrderNote } from '@/app/actions/order-notes'
...
export function OrderNotesSection({
  ...
  // Sync notes when initialNotes change (e.g., sheet re-opened)
  React.useEffect(() => {
  ...
  const handleCreate = async () => {
    ...
    // Optimistic update
    const optimisticNote: OrderNoteWithUser = {
      id: `temp-${Date.now()}`,
      order_id: orderId,
      workspace_id: '',
    ...
    try {
      const result = await createOrderNote(orderId, newNote.trim())
  ...
  const handleStartEdit = (note: OrderNoteWithUser) => {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const result = await updateOrderNote(editingId, editContent.trim())
  ...
  const canModify = (note: OrderNoteWithUser) => {
      ...
      <div className="space-y-6">
        ...
        {notes.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p>Sin notas</p>
            <p className="text-sm">Agrega notas para recordar detalles importantes sobre este pedido.</p>
          </div>
        ) : (
          <Timeline>
