/**
 * Somnio Recompra Agent — Comprehension Prompt
 *
 * System prompt for Claude Haiku structured output (Capa 2).
 * Fork of somnio-v3/comprehension-prompt.ts — client-aware version.
 *
 * Key differences from v3:
 * - CONTEXTO DE RECOMPRA section (client already bought before)
 * - confirmar_direccion intent for address confirmation gate
 * - Removed: contenido, formula, como_se_toma, efectividad intents
 * - Enhanced bot context rules for "Seria para la misma direccion?" flow
 */
...
- Usa el nombre del carrier COMO direccion (sin calle/carrera real)
- "centro oficina [ciudad]"
- "Principal Servientrega" (Somnio solo usa Inter, misma intencion)
- Variantes ortograficas: interrapidisimo, interrapidicimo, interapidisimo, intirrapicimo, rapidisimo, interrapid, iterrapidisimo
...
- Incluso si ya dio direccion completa

REGLA: Si dice "oficina" + "inter" → entrega_oficina. Si solo "inter" → menciona_inter.
NUNCA ambos true simultaneamente. En duda → menciona_inter (preguntar es mas seguro).
...
- confirmar_direccion: cuando el cliente confirma que la direccion precargada es correcta. Respuestas a "Seria para la misma direccion?": si, dale, esa misma, correcto, a la misma
...
- acknowledgment: reconocimientos puros sin contenido sustancial (ok, si, gracias, jaja, emojis solos). NUNCA usar para saludos. Si hay contexto claro del bot (pregunta sobre compra, confirmacion, direccion), usar el intent correspondiente (quiero_comprar, confirmar, seleccion_pack, confirmar_direccion)
...
- confirmar_direccion: el cliente confirma que la direccion precargada es correcta ("si", "dale", "esa misma", "correcto", "a la misma" — en respuesta a pregunta de direccion del bot)
