...
2. tags.ts: tag add/remove operations emit tag.assigned/tag.removed
3. contacts.ts: createContact emits contact.created
4. tasks.ts: task completion emits task.completed
5. webhook-handler.ts: incoming messages emit whatsapp.message_received
6. All emissions are fire-and-forget (no await, no try/catch wrapping)
7. Previous state captured before updates (for stage change previousStageId)
