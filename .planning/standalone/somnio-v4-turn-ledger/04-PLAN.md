...
must_haves:
  truths:
    ...
    - "Path A (interrupt zero-sends) NO persiste las dims (descarta turno, P6)"
    - "commitTurn emite cada entrada del ledger como evento agent_observability_events (kb_topic_registered, crm_action_recorded)"
  artifacts:
    - path: "src/lib/agents/engine/v4-production-runner.ts"
      provides: "restore + persist PATH B + carryState de turn_ledger_dims; emit observability"
...
cada entrada del ledger como evento de observabilidad para queryability cross-sesión (D-13).
...
Output: runner + engine-v4 + SandboxState threading; commitTurn (o el runner) emite eventos.
...
- P6 Path A: el saveState de Path A (runner :948-957) NO incluye dims; solo PATH B normal (:967-972).
...
- Restore acciones_ejecutadas: lee `rawState.acciones_ejecutadas` (:303-304), fallback _v3: key (:307-318). → AGREGAR lectura paralela de `rawState.turn_ledger_dims` (default {atendido:[],crmActions:[]}).
- seed object (:327-334) carga accionesEjecutadas (:334). → AGREGAR turnLedgerDims al seed.
...
- PATH A saveState (:948-957): acciones_ejecutadas:output.accionesEjecutadas (:956). → NO AGREGAR dims aquí (P6: Path A descarta turno).
- PATH B saveState (:967-972): acciones_ejecutadas (:971). → AGREGAR `turn_ledger_dims: output.turnLedgerDims`.
...
ya usado por crm-mutation-tools (3 eventos) y otros agentes. commitTurn (state.ts) o el
runner debe emitir `kb_topic_registered` (por cada atendido kind:'kb_topic') y
`crm_action_recorded` (por cada crmActions entry). Decisión de implementación: emitir desde
el runner post-commit (tiene acceso al collector/observability adapter) es más limpio que
desde commitTurn puro (state.ts no debe tener side-effects de I/O). Preferir emit en runner.
</interfaces>
</context>
...
<task type="auto">
  ...
  <action>
    1. Restore: tras leer acciones_ejecutadas (:303-318), leer `const sessionTurnLedgerDims = (rawState.turn_ledger_dims as TurnLedgerDims | undefined) ?? { atendido: [], crmActions: [] }` (default graceful — sesiones legacy sin columna o con `{}`).
    ...
    6. PATH B saveState (:967-972): agregar `turn_ledger_dims: output.turnLedgerDims`.
    7. PATH A saveState (:948-957): NO agregar dims (P6 — Path A descarta turno; persistirlas registraría efectos de un turno no-enviado).
    Importar TurnLedgerDims desde somnio-v4/types. NO introducir createAdminClient nuevo.
  </action>
  <verify>
    <automated>grep -n "turn_ledger_dims\|turnLedgerDims" src/lib/agents/engine/v4-production-runner.ts && ! grep -n "turn_ledger_dims" src/lib/agents/engine/v4-production-runner.ts | grep -E "94[89]|95[0-7]" && npx tsc --noEmit 2>&1 | grep "v4-production-runner" || echo "ok"</automated>
  </verify>
  <done>Runner restaura dims con default graceful, las pasa a V4AgentInput, las incluye en carryState (ambos sitios), las persiste en PATH B saveState, y NO en PATH A. Compila.</done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    1. `src/lib/sandbox/types.ts` SandboxState: agregar `turnLedgerDims?: TurnLedgerDims` (junto a accionesEjecutadas:257).
  ...
  <done>SandboxState tiene turnLedgerDims; engine-v4 lo restaura, lo carga en carryState + newState, y lo propaga a DebugTurn (vía newState). Compila. Paridad con runner.</done>
</task>

<task type="auto">
  <name>Task 3: Emitir eventos de observabilidad por entrada del ledger (kb_topic_registered, crm_action_recorded)</name>
  <files>src/lib/agents/engine/v4-production-runner.ts</files>
  <action>
    Post-commit (PATH B, después del saveState :967-972 exitoso), emitir a observability
    (mismo collector/adapter que ya usa el runner para pipeline_decision) un evento por cada
    entrada del ledger (D-13 — almacén analítico cross-sesión separado del blob per-sesión):
  ...
  <verify>
    <automated>grep -n "kb_topic_registered\|crm_action_recorded" src/lib/agents/engine/v4-production-runner.ts && npx tsc --noEmit 2>&1 | grep "v4-production-runner" || echo "ok"</automated>
  </verify>
  <done>El runner emite kb_topic_registered por cada kb_topic y crm_action_recorded por cada crmAction, solo en PATH B, con payload metadata (sin texto completo, args redactados). Compila.</done>
</task>
...
| T-run-02 | Repudiation (efecto de turno descartado) | Path A persiste dims | mitigate | Path A saveState NO incluye dims (P6) |
...
- P6: las dims NO aparecen en el saveState de Path A (líneas ~948-957).
- `grep "kb_topic_registered\|crm_action_recorded"` presente en runner.
...
(reprocess no pierde ni re-registra, P3); Path A NO persiste (P6); commitTurn no muta frontera
transaccional; el runner emite kb_topic_registered + crm_action_recorded en PATH B. interruption-system-v2 intacto.
</success_criteria>
...
- `feat(v4-ledger): emit kb_topic_registered + crm_action_recorded a observability (D-13)`
