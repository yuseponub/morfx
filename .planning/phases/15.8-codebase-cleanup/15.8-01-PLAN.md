...
must_haves:
  truths:
    ...
    - "Bug #8 verified as correct two-step signal: cancel ingest timer then start promo timer, with code comments explaining the design"
  ...
  artifacts:
    ...
    - path: "src/lib/sandbox/sandbox-engine.ts"
      provides: "Immutable state handling in checkImplicitYes and handleIngestMode, plus documented timer signal flow"
  ...
  key_links:
    ...
    - from: "sandbox-engine.ts processMessage"
      to: "timer signal flow"
      via: "two-step signal: handleIngestMode cancel -> processMessage start for promos"
      pattern: "justCompletedIngest.*timerSignal.*start"
---

<objective>
Fix all 6 critical severity bugs identified in the codebase audit: 3 stale closure bugs in sandbox-layout.tsx, 1 state mutation bug in sandbox-engine.ts, 1 stale closure verification in ingest-timer.ts, 1 timer signal verification in sandbox-engine.ts, and 1 race condition in message-sequencer.ts.
...
<task type="auto">
  <name>Task 2: Fix state mutation in sandbox-engine.ts (Bug #4)</name>
  <files>src/lib/sandbox/sandbox-engine.ts</files>
  <action>
Fix the state mutation bug in sandbox-engine.ts. This is the most complex fix in the plan and requires careful tracing of the caller code path.
...
After making changes, run `npx tsc --noEmit` to verify TypeScript compiles clean.
Commit: `fix(15.8-01): state mutation bug #4 in sandbox-engine.ts`
  </action>
  <verify>
1. `npx tsc --noEmit` passes with zero source errors
2. For Bug #4: Verify state mutation is either refactored to return new objects OR documented as intentional with safety comments
3. Trace the full code path: `checkImplicitYes` -> `processMessage` -> `justCompletedIngest` to confirm mode transitions still work correctly
  </verify>
  <done>
...
- processMessage correctly detects mode transitions after the change
- TypeScript compiles clean
  </done>
</task>

<task type="auto">
  <name>Task 3: Verify timer signal + stale closure + fix race condition (Bugs #5, #6, #8)</name>
  <files>
    src/lib/sandbox/sandbox-engine.ts
    src/lib/sandbox/ingest-timer.ts
    src/lib/agents/somnio/message-sequencer.ts
  ...
  <action>
Verify 2 bugs and fix/document 1 bug across 3 files.

**Bug #8 (timer signal override in sandbox-engine.ts - VERIFY AND DOCUMENT):**

The audit classifies this as a HIGH severity "Logic Error" where `timerSignal = cancel` at line 537 gets overridden by `timerSignal = start` at line 316. However, the research (and the existing code comments) indicate this is intentional two-step behavior.

**Verification steps (MUST complete all):**
1. Read `sandbox-engine.ts` lines 531-537 where `handleIngestMode` returns with `timerSignal = { type: 'cancel' }` on ingest completion.
2. Read lines 311-316 in `processMessage` where `justCompletedIngest && newMode === 'ofrecer_promos'` sets `timerSignal = { type: 'start' }`.
3. Confirm the TWO signals refer to DIFFERENT timer purposes:
   - The `cancel` at line 537 cancels the **ingest data-collection timer** (levels 1-2)
   - The `start` at line 316 starts the **promo-timeout timer** (level 3: promos sin respuesta)
4. Verify by checking the `lastTimerSignal` field: the cancel is consumed first (clearing the ingest timer), then the start initiates the promo timer. They are sequential, not conflicting.

**If confirmed as correct (expected outcome):**
Add a multi-line comment block at BOTH signal assignment sites explaining the design:

At lines 531-537 (the cancel):
```typescript
// TIMER SIGNAL: Cancel ingest timer (levels 1-2).
// When ingest completes, we first cancel the data-collection timer.
// processMessage will then start the promo timer (level 3) if transitioning to ofrecer_promos.
// See processMessage ~line 316 for the second step of this two-step signal.
```

At lines 311-316 (the start override):
```typescript
...
// The cancel cleared the ingest timer; this starts the promo-timeout timer.
// This is a two-step signal, NOT a bug. See handleIngestMode ~line 537 for step 1.
```

**If NOT correct (cancel and start refer to the same timer level):**
Fix by ensuring the cancel applies to the ingest level and the start applies to the promo level. Add level information to the timer signal.
...
Commit: `fix(15.8-01): verify timer signal #8, closure #5, document race condition #6`
  </action>
  <verify>
1. `npx tsc --noEmit` passes with zero source errors
2. For Bug #8: Grep confirms explanatory comments exist at BOTH signal sites: `grep -n "two-step signal\|TIMER SIGNAL" src/lib/sandbox/sandbox-engine.ts` should return 2+ matches
3. For Bug #5: Verify contextProvider reads from stateRef.current
4. For Bug #6: Verify interruption check has documentation about cache staleness
  </verify>
  <done>
- Bug #8: Timer signal two-step flow (cancel ingest -> start promos) confirmed as correct design; multi-line comments added at both signal sites explaining the intent
...
5. Bug #8 has explanatory comments at both signal assignment sites
</verification>

<success_criteria>
1. Bugs #1, #2, #3 fixed with useRef pattern in sandbox-layout.tsx
2. Bug #4 addressed (refactored or documented) in sandbox-engine.ts as a standalone task
3. Bug #5 verified in ingest-timer.ts
4. Bug #6 documented/improved in message-sequencer.ts
5. Bug #8 verified as intentional two-step signal with comments at both sites in sandbox-engine.ts
