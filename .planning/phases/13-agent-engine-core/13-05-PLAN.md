 ...
 * 5. Record user turn
 * 6. Route based on confidence (handoff/clarify/proceed)
 * 7. Orchestrate response (with Sonnet, tools available)
 * 8. Execute tools if requested
 * 9. Update session state with version check
    ...
    try {
      ...
      try {
        await this.sessionManager.updateSessionWithVersion(
          ...
          session.version,
          {
            currentMode: orchestratorResult.nextMode ?? session.current_mode,
