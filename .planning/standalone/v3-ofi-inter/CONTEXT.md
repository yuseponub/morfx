...
- Auto-triggers ad-hoc (el anterior `ciudad_sin_direccion` se ELIMINA)
...
### Comprehension: deteccion de señales

- El campo actual `ofi_inter: boolean` en comprehension-schema.ts necesita expandirse para distinguir Señal 1 vs Señal 2
- Opciones: dos campos separados (`entrega_oficina` + `menciona_inter`), o un campo con mas granularidad
- El prompt de comprehension debe cubrir TODAS las variantes ortograficas reales
- Distincion clave para el prompt: "oficina/recoger/sede + inter" = entrega_oficina. Solo "inter" sin oficina = menciona_inter.
