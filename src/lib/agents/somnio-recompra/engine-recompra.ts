/**
 * Somnio Recompra Engine — Minimal Sandbox Runner
 *
 * Thin engine for sandbox-only recompra agent testing.
 * Handles bidirectional mapping: SandboxState <-> AgentState
 * via _v3: prefixed keys in datosCapturados.
 *
 * Fork of somnio-v3/engine-v3.ts — same pattern, local imports.
 */

import { processMessage } from './somnio-recompra-agent'
...
export class SomnioRecompraEngine {
  async processMessage(input: V3EngineInput): Promise<V3EngineOutput> {
    ...
    try {
      ...
      console.error('[SomnioRecompraEngine] Error:', error)

      return {
        success: true,
        messages: [`[Error recompra] ${errorMsg}`],
