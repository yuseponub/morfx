...
/**
 * Get all notes for an order, sorted by created_at DESC (newest first)
...
export async function updateOrderNote(noteId: string, content: string): Promise<ActionResult<void>> {
  ...
  // Get the note first to check permissions and get workspace_id
  ...
  const result = await domainUpdateOrderNote(ctx, { noteId, content: trimmedContent })
...
export async function deleteOrderNote(noteId: string): Promise<ActionResult<void>> {
  ...
  // Delegate to domain
  const ctx: DomainContext = { workspaceId: note.workspace_id, source: 'server-action' }
  const result = await domainDeleteOrderNote(ctx, { noteId })
