...
- [ ] 41-13-PLAN.md — GAP-41-08 gap-closure: reclasificar mp4/mov audio-only a audio en envíos IG/FB (pure helper `isAudioOnlyMp4` + reclasificación server-side gateada a IG/FB) — IG-02 — **PLANNED 2026-06-06** (TDD: audio-only .mp4 con `soun`/sin `vide` → `mediaType='audio'` → Meta 200; .mp4 con `vide` → sigue video; non-mp4/garbage → false. Scan acotado 512KB; WhatsApp sin gate (Regla 6); helper puro sin createAdminClient (Regla 3); sin migración (Regla 5). Wave 1 gap-closure, depends_on []. Next: `/gsd-execute-phase 41 --gaps-only`)
