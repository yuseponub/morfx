...
must_haves:
  truths:
    ...
    - "When the domain atomically marks a job as completed (status='completed'), the callback API emits robot/job.batch_completed to Inngest"
...
6. **Check if batch completed -> emit batch_completed** -- Re-read the job status AFTER the domain update. The domain layer atomically sets `status = 'completed'` when `success_count + error_count >= total_items`, so we use that as the authoritative guard instead of doing counter arithmetic ourselves (avoids spurious duplicate events from concurrent final callbacks):
```typescript
  // Re-read job status after domain update — domain sets status='completed' atomically
