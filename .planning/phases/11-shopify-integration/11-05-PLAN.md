...
NOTE: The existing middleware.ts already passes webhook routes through without body parsing (lines 8-12: `if (pathname.startsWith('/api/webhooks')) return NextResponse.next()`). No middleware changes needed.
  </action>
  <verify>Verify route compiles. Test with curl: `curl -X GET http://localhost:3020/api/webhooks/shopify` should return status: "ok".</verify>
