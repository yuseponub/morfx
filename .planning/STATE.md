...
status: ready
stopped_at: "Standalone `somnio-v4-rag-generative` Plan 05 (Smoke A) **SHIPPED 2026-05-18**. 4 commits Plan 05 (`165a82f` Task 5.1 judge + `092e5d8` Task 5.2 smoke test + `ab7a8a1` Task 5.3 throttle 7s + docs commit). Smoke A 17/17 evaluado clean (paid tier Gemini, `serviceTier=standard` verificado pre-run). **Resultado judge:** 14/17 PASS (82.4%), 0 FAIL PARTIAL, 3/17 FAIL (cases 2 embarazo + 13 duracion_efecto + 14 habitos_sueno — todos patrón único `nuncaDecirCheck` false-positive). **0/17 invenciones** detectadas por judge — RAG architecture sin alucinaciones, faithfulness 100%. Calibration 14/17 CALIBRATED + 3/17 MISCALIBRATED_HIGH (mismas 3 FAILs). Runtime errors 0/17 — paid tier eliminó bloqueo 2026-05-17. Avg latency 37.1s/caso (tooling+generation+judge), run duration 793s vitest. Negativos manejados bien (2/3 handoff silente + 1/3 respuesta constructiva con material adyacente — case 17 cripto usó `pago` topic). Topic selection 100% on-target. Threshold 0.70 (D-14) validado: 14 cases con conf 0.95 + 2 cases con conf ≤0.40 handoff. **Hallazgo crítico:** `nuncaDecirCheck` guardrail dispara false-positives con string match plano que ignora polaridad — Plan 07 candidate (3 opciones documentadas en SMOKE-A-RESULTS.md). v4 sigue DORMANT en prod (`routing_rules` sin regla v4, Regla 6 honored — v3 atendiendo clientes). Next: Jose revisa 17 casos manualmente en SMOKE-A-RESULTS.md (Jose final ☐), luego decide camino A (Plan 07 iter antes de Smoke B — recomendado) o B (avanzar Plan 06 con riesgo de FAILs recurrentes)."
