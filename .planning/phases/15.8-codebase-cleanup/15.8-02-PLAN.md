...
4. Security #2 wiring: `grep -n "request.json()" src/app/api/webhooks/whatsapp/route.ts` returns ZERO matches (all body parsing must use `JSON.parse(rawBody)` after HMAC verification reads raw body first)
5. Security #3: `grep -n "workspace_members.*role" src/app/actions/invitations.ts` shows membership checks in 3 functions
  </verify>
  <done>
