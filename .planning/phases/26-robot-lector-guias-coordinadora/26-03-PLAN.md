...
0. **CRITICAL one-line fix:** Update the existing `executeSubirOrdenesCoord` function's `getActiveJob(ctx)` call to `getActiveJob(ctx, 'create_shipment')`. This ensures a running guide_lookup job does NOT block shipment creation, and vice versa. Without this change, the `must_have` "Guide lookup and shipment creation jobs don't block each other" is NOT achieved.
...
- executeSubirOrdenesCoord updated: getActiveJob(ctx) changed to getActiveJob(ctx, 'create_shipment') — shipment and guide jobs no longer block each other
