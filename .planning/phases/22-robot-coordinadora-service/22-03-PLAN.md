...
must_haves:
  truths:
    ...
    - "Re-submitting the same jobId returns a cached 200 response (idempotency)"
    - "Orders with empty or missing ciudad are rejected before opening a browser session"
    - "Each order in the batch is processed sequentially with 2-second delays between orders"
    - "After each order, the robot POSTs the result to the callbackUrl provided in the request (using result.numeroPedido for trackingNumber)"
...
**Module-level state (outside createServer):**

```typescript
// Idempotency cache: prevents re-processing the same jobId on sequential re-submissions.
// Keyed by jobId, stores the immediate acknowledgement response.
// The workspace lock prevents CONCURRENT duplicates; this cache prevents SEQUENTIAL re-submissions.
...
b) **Check jobId idempotency cache:**
   - If `completedJobs.has(jobId)`, return 200 with the cached `BatchResponse`
   - This prevents re-processing when MorfX retries or Inngest replays the same job

c) **Check workspace lock:**
...
d) **Pre-validate orders (lightweight city check -- ROBOT-02):**
   - Before acknowledging, iterate over `orders` and check each `order.pedidoInput.ciudad`
   - If `ciudad` is empty string, undefined, or null: report this order immediately via callback as error (`errorType: 'validation'`, `errorMessage: 'Ciudad vacía o no proporcionada'`) and REMOVE it from the orders array (do not send to the portal)
   - If ALL orders are rejected by this check, return 400 with `{ success: false, error: 'Todos los pedidos tienen ciudad inválida' }`
   - This is a lightweight sanity check. Full city coverage validation happens in Phase 23 (MorfX validates before dispatch). The robot only rejects obviously invalid data to avoid wasting a browser session.

e) **Acknowledge immediately:**
   - Build the response: `{ success: true, jobId, message: 'Batch aceptado, procesando...' }`
   - Store in `completedJobs.set(jobId, response)` BEFORE sending (idempotency)
   - Return 200 with the response
   - Processing continues in the background (do NOT await the processing promise before responding)

f) **Process batch in background** (async, after response is sent):
   ...
   - For each order in `orders` (only those that passed the city pre-check):
     ...
     - Report result to callback URL. **CRITICAL field mapping:** use `result.numeroPedido` (NOT `result.numeroGuia`) for the `trackingNumber` field in the callback payload -- this matches the `GuiaResult` type from `types/index.ts`
...
**Mapping GuiaResult to BatchItemResult** (used when building the callback payload):
```typescript
// CRITICAL: Use result.numeroPedido (from GuiaResult type), NOT result.numeroGuia
const callbackPayload: BatchItemResult = {
  itemId: order.itemId,
  status: result.success ? 'success' : 'error',
  trackingNumber: result.numeroPedido,   // <-- MUST be numeroPedido
...
- `grep "completedJobs" robot-coordinadora/src/api/server.ts` finds the idempotency cache (ROBOT-05)
- `grep "ciudad" robot-coordinadora/src/api/server.ts` finds the lightweight city pre-validation (ROBOT-02)
- `grep "numeroPedido" robot-coordinadora/src/api/server.ts` confirms the callback maps `result.numeroPedido` (not `numeroGuia`)
- `grep "409" robot-coordinadora/src/api/server.ts` confirms the workspace lock returns 409 Conflict
  </verify>
  <done>
Express server handles health checks and batch processing with workspace/order locking, jobId idempotency cache (prevents sequential re-submissions), lightweight city pre-validation (rejects empty ciudad), sequential order processing with delays, callback result reporting using correct `numeroPedido` field, and proper error handling for all failure modes including 409 Conflict for active batches.
  </done>
</task>
...
7. POST /api/crear-pedidos-batch with a duplicate jobId returns the cached 200 (idempotency)
8. POST /api/crear-pedidos-batch while another batch runs returns 409 Conflict
9. POST /api/crear-pedidos-batch with orders containing empty ciudad rejects those orders via callback before opening browser
10. Callback payload uses `result.numeroPedido` for `trackingNumber` (not `numeroGuia`)
11. Dockerfile references correct Playwright base image version
12. HEALTHCHECK is configured for /api/health
13. All imports resolve correctly (types -> adapter -> server -> index chain)
</verification>

<success_criteria>
- Express server starts on port 3001 and responds to health checks
- Batch endpoint validates input, checks jobId idempotency, checks workspace lock, pre-validates city fields, acknowledges immediately, and processes in background
- Callback reporting POSTs results to the provided callbackUrl using `result.numeroPedido`
- Orders with empty ciudad are rejected before wasting a browser session
- Re-submitting the same jobId returns a cached response
