...
The callback route needs to be extended to handle guide lookup results. The key change is in section 4 (domain update) and section 5 (automation trigger):

1. After looking up the item (section 3), ALSO look up the parent job's `job_type`:
   ```typescript
   // Look up parent job type for routing field updates
...
2. The domain layer (`updateJobItemResult` from Plan 01) already handles the routing based on job_type. The callback just needs to pass the data through. No changes needed to the `updateJobItemResult` call — it already receives `trackingNumber` and the domain function (modified in Plan 01) checks the parent job's type to decide whether to write `tracking_number` or `carrier_guide_number`.

3. In section 5 (trigger emission), the existing code only fires `emitRobotCoordCompleted` for `status === 'success' && trackingNumber`. For guide_lookup jobs, we want to fire `emitFieldChanged` instead (which happens automatically through `updateOrder` in the domain layer from Plan 01). So the trigger emission in the callback route should be SKIPPED for guide_lookup jobs:
