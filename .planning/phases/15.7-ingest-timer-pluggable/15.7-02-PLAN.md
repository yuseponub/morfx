...
<task type="auto">
  ...
  <action>
**Complexity note:** This task has high complexity (10+ concerns: timer lifecycle, state management, signal processing, expiration handling, level chaining, order creation routing, config changes, toggle, pause/resume, reset integration, and prop passing). This is accepted risk because: (a) the action below is very detailed with explicit code snippets, (b) this is sandbox-only code with no production impact, and (c) all concerns modify the same file so splitting would create dependency issues.
...
const handleTimerExpire = useCallback((level: number, action: TimerAction) => {
  ...
  if (action.type === 'create_order') {
    ...
    // TODO: CRM orchestrator integration can be added later for real order creation
  }
...
Note: Order creation uses a placeholder message. CRM orchestrator integration for real order creation can be added in a later phase.
...
**File 3: ingest-tab.tsx — Major rewrite of TimerControls and TimerCountdown**
