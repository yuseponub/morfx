...
Phase: 42.1 (Observabilidad Bots Produccion) — IN PROGRESS (Wave 4 — Plan 08 COMPLETE)
Plan: 8/11
Status: Wave 4 Plan 08 COMPLETE. `src/inngest/functions/observability-purge.ts` creado como clon estructural exacto de `close-stale-sessions.ts` (Phase 42 Plan 02): `observabilityPurgeCron` con `retries: 1`, cron `TZ=America/Bogota 0 3 * * *` (03:00 Bogota, +1h offset vs close-stale @ 02:00 para evitar overlap de presion DB en ventana nocturna). Dos step.run en serie: (1) `ensure-next-month-partition` calcula primer dia del mes siguiente y llama al RPC `create_observability_partition(target_month)` del Plan 01 (idempotente: no-op si ya existe, evita fallo el dia 1 del mes); (2) `drop-old-partitions` calcula `cutoff = hoy - 30 dias` floored a inicio de mes y llama `drop_observability_partitions_older_than(cutoff)` — el floor a inicio de mes previene drop prematuro dentro de la ventana de retencion util. Cada step usa `createAdminClient()` inline + `supabase.rpc(...)` + `logger.error` + `throw error` para que Inngest reintente una vez. Logger final emite `{ createdFor, dropped, cronRunAt }` con `createModuleLogger('observability-purge')` (pino). **Feature flag OFF safety:** el cron opera sobre schema directo via RPC, no depende del collector AsyncLocalStorage ni de `runWithCollector`; corre FUERA de cualquier turn context asi que el `createAdminClient()` instrumentado no tiene collector activo donde recursar (cero Pitfall 1). Con dataset vacio, Step 1 ensure es no-op y Step 2 drop retorna array vacio — inofensivo. **Registro:** `src/app/api/inngest/route.ts` import + entry en array `functions` junto a `taskOverdueCron` y `closeStaleSessionsCron`. **Verificacion:** `tsc --noEmit` clean en ambos archivos (unicos errores en el repo son vitest en tests de somnio, pre-existentes). `pnpm run build` no ejecutado por outage persistente de Google Fonts en WSL documentado en Plan 07 — tsc clean + paridad estructural con cron existente en prod consideradas verificacion suficiente. **Paridad con close-stale-sessions:** imports identicos, `retries: 1`, cron con prefijo inline `TZ=`, step.run throw-on-error, logger.info final con cronRunAt, registro en el mismo array de route.ts. Unica diferencia semantica: 2 steps vs 1 (operaciones independientes que Inngest puede reintentar por separado). Wave 4 COMPLETE con Plans 07+08. Wave 5 (UI/dashboards) desbloqueada.
Last activity: 2026-04-07 — Plan 08 ejecutado. Commits 2f5e96b (observability-purge.ts) + 337b1cc (route.ts registracion).
...
| 42.1 | Observabilidad Bots Produccion (mirroring + deep logging) | Operational urgency | IN PROGRESS (7/11 plans — Wave 4 Plan 07: flush + wire COMPLETE) |
