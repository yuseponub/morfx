...
2. `processMessageWithAgent(input: ProcessMessageInput)` — Main function:
   ...
   d. **Broadcast typing indicator START via Supabase Realtime:**
      - Create a Supabase admin client channel: `supabase.channel(\`agent-typing:${conversationId}\`)`
      - Send broadcast: `channel.send({ type: 'broadcast', event: 'agent_typing', payload: { isTyping: true } })`
      - This enables the "Bot escribiendo..." indicator in Plan 03's chat-view

   e. Create SomnioEngine instance with workspaceId

   f. Call `engine.processMessage()` with SomnioProcessMessageInput

   g. **Broadcast typing indicator STOP via Supabase Realtime:**
      - Send broadcast: `channel.send({ type: 'broadcast', event: 'agent_typing', payload: { isTyping: false } })`
      - Unsubscribe from channel after sending
      - Wrap in try/finally so typing=false is ALWAYS sent, even if processing throws

   h. After processing, mark messages sent by agent:
   ...
   i. **CRITICAL: Re-check agent enabled BEFORE confirming** (Pitfall 3)
   ...
   j. If result.newMode === 'handoff', call executeHandoff()

   k. Log completion with structured info (conversationId, success, messagesSent, tokensUsed)
...
Use createAdminClient for all DB operations. Use createModuleLogger for structured logging.
  ...
  <verify>TypeScript compiles without errors. Verify processMessageWithAgent imports SomnioEngine correctly. Verify processMessageWithAgent broadcasts typing indicator (isTyping true before processing, false after). Verify handoff disables only conversational agent. Verify round-robin queries team_members correctly.</verify>
  <done>Webhook processor routes messages through SomnioEngine with agent-config checks, auto-contact creation, sent_by_agent marking, and Realtime typing broadcasts. Handoff handler creates tasks with round-robin and disables only conversational agent.</done>
</task>
...
- [ ] processMessageWithAgent broadcasts typing indicator via Supabase Realtime (true before, false after)
