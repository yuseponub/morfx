// Cloned verbatim from src/lib/agents/godentist/transitions.ts (Standalone: agent-godentist-fb-ig, Wave 1 Plan 02).
...
export const TRANSITIONS: TransitionEntry[] = [
  ...
  // Rule 8: seleccion_sede + !datosCriticos -> pedir_datos_con_sede + L0
  {
    phase: 'initial', on: 'seleccion_sede', action: 'pedir_datos_con_sede',
    condition: (_, gates) => !gates.datosCriticos,
    resolve: () => ({
      timerSignal: { type: 'start', level: 'L0', reason: 'sede elegida, pidiendo datos restantes' },
      reason: 'Sede elegida en initial, pedir datos restantes',
    }),
    description: 'Rule 8: initial + seleccion_sede + !datosCriticos -> pedir_datos_con_sede + L0',
  },
  ...
  // Rule 25: seleccion_sede + !datosCriticos -> pedir_datos_con_sede + L0
  {
    ...
    description: 'Rule 25: capturing_data + seleccion_sede + !datosCriticos -> pedir_datos_con_sede + L0',
  },
