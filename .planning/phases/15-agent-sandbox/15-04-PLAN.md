...
<task type="auto">
  ...
  <action>
Update `src/app/(dashboard)/sandbox/components/sandbox-layout.tsx` to integrate debug panel and session controls:
...
Update `src/app/(dashboard)/sandbox/components/sandbox-header.tsx` to include session controls:
...
<task type="auto">
  ...
  <action>
Read the current sidebar.tsx file and add a sandbox link in the appropriate section.

The sandbox link should be added with:
- Icon: Bot (from lucide-react)
- Label: "Sandbox"
- Path: "/sandbox"
- Position: After existing main navigation items, before settings/configuration

Add the import for Bot icon and add the navigation item. The link should be visible to all authenticated users since it's a developer/testing tool.

Look at the existing structure and add:
```typescript
{
  href: '/sandbox',
  icon: Bot,
...
In the appropriate navigation array after the main CRM/WhatsApp links but before any settings links.
  </action>
  <verify>
Check that sidebar.tsx compiles without errors.
Verify Bot icon is imported from lucide-react.
Check the /sandbox link appears in the correct position.
  </verify>
  <done>
Sidebar navigation includes Sandbox link.
