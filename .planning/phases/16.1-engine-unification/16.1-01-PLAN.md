...
must_haves:
  truths:
    - "5 adapter interfaces (Storage, Timer, Messaging, Orders, Debug) are defined with correct method signatures"
    - "EngineInput and EngineOutput types cover all fields needed by both sandbox and production flows"
    - "EngineConfig type allows per-environment timer durations, response speed, and CRM modes"
    - "AgentSessionLike type satisfies both in-memory sandbox state and DB-backed production session"
  artifacts:
