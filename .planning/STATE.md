...
status: ready
stopped_at: "Standalone `somnio-v4-rag-generative` Wave 2 (Plans 02 + 03 atómicos) **SHIPPED 2026-05-16**. 13 commits totales pre-push (4 Plan 02 `33f3f83..a8313b1` + 9 Plan 03 `2add7ca..a74a3b5`) + 1 docs commit pending. Plan 02: 18 KBs reescritos al formato RAG-generative (5 secciones, prefijos [SIEMPRE]/[SI APLICA], NUNCA decir preservado, casos críticos Smoke A — lupus/sertralina/Miami/criptomonedas — defensivos). Plan 03: sub-loop split tooling/generación arquitectura. 4 archivos nuevos (safe-output.ts, tone-base.ts, tooling-call.ts, generation-call.ts) + 7 refactored (index.ts orchestrator switch por reason, output-schema status enum 'generated' reemplaza 'canonical' D-24, prompt.ts buildToolingPrompt + buildGenerationPrompt + FewShot, kb-search-tool KbHit con 5 cols nuevas, debug-payload con toolingCall + generationCall snapshots, somnio-v4-agent consumer + subloop-tab debug panel). M3 binary backstop enum (RESPONDE_BIEN/FALTA_INFO/FUERA_SCOPE), threshold 0.70 (D-19), safetySettings BLOCK_NONE x4 Pitfall 6. nunca-decir-check.ts + comprehension-schema.ts INTACTOS (D-09/D-25 locks). 29 unit tests passed + 2 E2E gated + 6 integration skipped (superseded). v4 sigue dormant en prod (`active_v4_rules = 0`, Regla 6 intocado). **Open debt:** Plan 02 Task 2.4 (DB sync) DEFERIDO por auth-gate Vercel — usuario debe correr `pnpm knowledge:sync` antes de Plan 05. Next: `/gsd-execute-phase somnio-v4-rag-generative --wave 3` para Plan 04 (few-shots calibración Gemini)."
...
Phase: Standalone `somnio-v4-rag-generative` — **Wave 2 (Plans 02 + 03) SHIPPED 2026-05-16**. Plans 01 + 02 + 03 todos cerrados. Sub-loop refactor RAG-generative ARQUITECTURA COMPLETA pre-push atómico final. **Plan 02 commits (4):** `33f3f83` Task 2.1 edge-cases → `7a4ba97` Task 2.2 product → `eb825a6` Task 2.3 policies+faqs → `a8313b1` 02-SUMMARY. 18 KBs reescritos al formato RAG-generative (5 sections + tone_override + prefijos [SIEMPRE]/[SI APLICA] + casos críticos defensive para Smoke A: lupus/sertralina/Miami/criptomonedas/apnea). NUNCA decir preservado verbatim. 32/32 tests parser+coherence-check verdes. **Plan 03 commits (9):** `2add7ca` Task 3.1 safe-output.ts (A3 wrapper defensivo vercel/ai#11348) → `60bf4ef` Task 3.2 tone-base.ts (D-05 TONE_BASE global) → `c2f4993` Task 3.3 kb-search-tool con 5 cols nuevas → `91929db` Task 3.4 tooling-call.ts (Call 1 GPT-4o mini + kb_search) → `af5fce8` Task 3.5 generation-call.ts (Call 2 Gemini 2.5 Flash NORMAL + temp 0.3 + BLOCK_NONE x4 + M3 binary backstop) → `cf42cd6` Task 3.6 output-schema refactor (status enum 'generated' reemplaza 'canonical' D-24, responseText + responseConfidence + confidenceRationale + invariants actualizados) → `5ff1965` Task 3.7 prompt.ts split (buildToolingPrompt preserva crm_mutation/cas_reject D-12 + buildGenerationPrompt con anti-invención + M1 PROBABILIDAD + M2 5 buckets + M3 binary) → `a73b289` Tasks 3.8+3.9 orchestrator (runRagSubLoop + runLegacySubLoop) + debug-payload extended + consumers (somnio-v4-agent + subloop-tab.tsx) → `a74a3b5` Task 3.10 tests (safe-output 5 tests + output-schema 15 tests + sub-loop-e2e 4 syntactic + 2 E2E gated + integration tests SKIPPED superseded). **LOCKS verificados:** nunca-decir-check.ts (D-09) + comprehension-schema.ts (D-25) + sub-loop/tools.ts (D-12) INTACTOS via `git diff --name-only`. **TypeScript scope CLEAN** (`npx tsc --noEmit | grep -E "sub-loop|somnio-v4"` retorna 0 errors). **Open debt CRÍTICA:** Plan 02 Task 2.4 (DB sync — `pnpm knowledge:sync` para poblar las 5 columnas + regenerar embeddings) DEFERIDO por auth-gate Vercel CLI sin decryptar `OPENAI_API_KEY_SALESV4`. Usuario debe correr el sync con `.env.local` productivo ANTES de Plan 05 (Smoke A) o smoke retornará false-negatives (sub-loop recibe material null y emite handoff por todo). v4 sigue dormant en prod (`active_v4_rules = 0`, Regla 6 honored — v3 atendiendo clientes). Next active carryover: Phase 37.5-05 Meta Business Verification (awaiting user manual action — Meta Business Manager + Porkbun DNS).
Plan: somnio-v4-rag-generative-03 **COMPLETE** (atómico con Plan 02). Plan 04 (few-shots calibración) next en Wave 3.
Status: Plans 02 + 03 closed. Ready for Wave 3 (`/gsd-execute-phase somnio-v4-rag-generative --wave 3`) — pero **antes** ejecutar `pnpm knowledge:sync` con keys productivas para resolver Open debt (requerido para Plan 05 Smoke A, no para Plan 04).
