...
3. **v4 comprehension sin fallback ante saturacion de Gemini — ✅ RESUELTA para v4 (standalone `gemini-fallback-haiku`, 2026-06-11)** — El problema original: la comprension de `somnio-sales-v4` (y los demas call-sites Gemini de v4) usaba UNICAMENTE `google('gemini-2.5-flash')` con los reintentos default del AI SDK (3 intentos lentos) y SIN modelo de fallback; cuando Google devolvia "This model is currently experiencing high demand" (saturacion transitoria del proveedor), los reintentos fallaban y el turno moria con `AI_RetryError` → Intent="error", sin respuesta. Observado en sandbox 2026-05-28; tumbo casos en los smokes del gate de `somnio-v4-consolidation` (GATE-W2.md).

   **Resolucion (standalone `gemini-fallback-haiku`):** modulo de fallback acotado a v4 en `src/lib/agents/somnio-v4/llm-fallback/` que envuelve los **4 call-sites Gemini de v4** via `callWithGeminiFallback`:
   - `comprehension.ts` (clasificacion de intent), `sub-loop/generation-call.ts` (redaccion RAG), `sub-loop/compliance-check.ts` (verifier nunca_decir), `media/image-classifier.ts` (vision).
   - **Deteccion N=1 (D-05):** `maxRetries: 0` en el branch Gemini → el PRIMER fallo de saturacion (predicado `isGeminiSaturation`: APICallError 503/429/500/504 + mensajes "high demand"/"MODEL_CAPACITY_EXHAUSTED") dispara el fallback en la MISMA llamada; el cliente no paga la latencia acumulada de 3 retries.
   - **Circuit-breaker in-memory (D-07/D-08):** FSM module-singleton por call-site con cooldown 30s + probe half-open con trafico real; mientras esta abierto, las llamadas van directo al fallback sin reintentar Gemini.
   - **Fallback con techo absoluto Haiku 4.5 (D-02):** `anthropic('claude-haiku-4-5')` via `@ai-sdk/anthropic` directo — **NUNCA** via el wrapper legacy `claude-client.ts` (que mapea `claude-haiku-4-5`→Sonnet). NUNCA escala a Sonnet/Opus (restriccion de costo).
   - **Paridad de shape (D-09):** mismo prompt + mismo Zod schema en ambos branches; el resto del pipeline (gates nunca_decir, threshold 0.70 → handoff) no se entera de que provider respondio. En comprehension se usa `MessageAnalysisSchemaSanitized` (sin `min`/`max`) para el branch Anthropic porque Anthropic rechaza esos bounds en JSON Schema, + clamp 0..1 post-parse (Pitfall #1).
   - **Observability (D-10):** typed-union de 6 labels `pipeline_decision` (`fallback_triggered`, `circuit_opened`, `circuit_closed`, `probe_ok`, `probe_failed`, `fallback_failed`) para auditar la frecuencia REAL de saturacion de Gemini en prod/sandbox.
   - **Vision (D-03):** el classifier cae a Haiku 4.5 con vision; el fail-safe handoff queda como ULTIMO recurso (solo si AMBOS providers caen).

   **Deuda que QUEDA (anotada):** (a) `tooling-call.ts` (GPT-4.1-mini) NO entra al fallback — no es Gemini, fuera de scope. (b) Generalizar el modulo a otros agentes (shared) es standalone futuro (D-04 lo acota a v4). (c) El mapping stale `claude-haiku-4-5`→Sonnet en `src/lib/agents/claude-client.ts:29` sigue VIVO sin tocar (Regla 6 — afecta consumidores legacy v3/godentist/recompra/pw-confirmation); limpiarlo es deuda separada. (d) P95 reales por call-site para afinar timeouts y smoke E2E con saturacion inyectada quedan diferidos a la activacion de v4 en prod. v4 sigue DORMANT en prod (0 workspaces) → el fallback no altera el comportamiento del agente activo; se ejercita solo ante saturacion real de Gemini.
