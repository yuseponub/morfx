...
- **Validación (gates verificables):**
  ...
  - **Contrato de paridad producción ↔ sandbox:** `src/lib/agents/somnio-v4/INTERRUPTION-PARITY.md` — explica cómo funciona el sistema en cada lado y la regla de oro: prod (`engine/v4-production-runner.ts`) y sandbox (`somnio-v4/engine-v4.ts`) NO comparten código pero DEBEN ir alineados en mecanismo (Path A/B, `dropOwnEntry`, `carryState`, restart loop). Leer antes de tocar la lógica de interrupción en cualquiera de los dos.
