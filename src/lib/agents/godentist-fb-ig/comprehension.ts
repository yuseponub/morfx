...
/**
 * GoDentist FB/IG Sibling Agent — Comprehension Layer (Capa 2)
 *
...
import { GD_INTENTS } from './constants'

const GD_INTENTS_SET = new Set<string>(GD_INTENTS)
...
export async function comprehend(
  ...
  const response = await runWithPurpose('godentist_fb_ig_comprehension', () =>
  ...
  if (!textBlock || textBlock.type !== 'text') {
    throw new Error('[Comprehension-gd] No text content in Claude response')
  }
...
function parseAnalysis(rawText: string): MessageAnalysis {
  ...
  try {
    ...
    throw new Error(`[Comprehension-gd] Invalid JSON from Claude: ${rawText.slice(0, 200)}`)
  }
  ...
  if (intent) {
    if (typeof intent.primary === 'string' && !GD_INTENTS_SET.has(intent.primary)) {
      console.warn(`[Comprehension-gd] Unknown intent.primary="${intent.primary}", falling back to "otro"`)
      intent.primary = 'otro'
    }
    if (typeof intent.secondary === 'string' && intent.secondary !== 'ninguno' && !GD_INTENTS_SET.has(intent.secondary)) {
      console.warn(`[Comprehension-gd] Unknown intent.secondary="${intent.secondary}", falling back to "ninguno"`)
  ...
  throw new Error(`[Comprehension-gd] Failed to parse after sanitization:\n${issues}`)
}
