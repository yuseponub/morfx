// Clonado VERBATIM de src/lib/agents/godentist-fb-ig/comprehension.ts (Standalone agent-varixcenter Wave 2 Plan 04 Task 1).
// Cambios: import del schema/prompt de varixcenter; log prefix renombrado a [varixcenter];
// purpose de observability godentist_fb_ig_comprehension -> varixcenter_comprehension;
// agent en recordEvent godentist-fb-ig -> varixcenter. Modelo sigue siendo Haiku (D-12).

/**
 * Varixcenter Agent — Comprehension Layer (Capa 2)
 *
 * Single Claude call with structured output.
 * Extracts intent, data fields, classification.
 *
...
const VARIX_INTENTS_SET = new Set<string>(VARIX_INTENTS)
 ...
 * @param recentBotMessages - Recent bot messages for short-response context
...
export async function comprehend(
  ...
  const response = await runWithPurpose('varixcenter_comprehension', () =>
  ...
  if (!textBlock || textBlock.type !== 'text') {
    throw new Error('[varixcenter] No text content in Claude response')
  }
...
function parseAnalysis(rawText: string): MessageAnalysis {
  ...
  try {
    ...
    throw new Error(`[varixcenter] Invalid JSON from Claude: ${rawText.slice(0, 200)}`)
  }
  ...
  if (intent) {
    if (typeof intent.primary === 'string' && !VARIX_INTENTS_SET.has(intent.primary)) {
      console.warn(`[varixcenter] Unknown intent.primary="${intent.primary}", falling back to "otro"`)
      intent.primary = 'otro'
    }
    if (typeof intent.secondary === 'string' && intent.secondary !== 'ninguno' && !VARIX_INTENTS_SET.has(intent.secondary)) {
      console.warn(`[varixcenter] Unknown intent.secondary="${intent.secondary}", falling back to "ninguno"`)
  ...
  throw new Error(`[varixcenter] Failed to parse after sanitization:\n${issues}`)
}
