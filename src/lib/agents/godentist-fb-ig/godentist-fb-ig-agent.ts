...
// console log prefix updated to "[GoDentist FB/IG]" for sibling-only debugging.
// processMessage entry point — re-exported from index.ts.

/**
 * GoDentist FB/IG Sibling Agent — Main Agent Pipeline
 *
  ...
  try {
    ...
    if (salesResult.accion === 'mostrar_disponibilidad' && mergedState.datos.fecha_preferida && mergedState.datos.sede_preferida) {
      try {
        ...
        console.error('[GoDentist FB/IG] Availability lookup failed (fail-open):', err)
    ...
    console.error('[GoDentist FB/IG] Error processing message:', errMsg)
    return {
      success: false,
      messages: [],
