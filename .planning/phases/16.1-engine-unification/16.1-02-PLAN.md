...
After handleIngestMode returns, the main processMessage checks `modeChanged` to set `justCompletedIngest = true`, instead of checking a mutation on `currentState`.
