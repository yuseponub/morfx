...
interface NotesSectionProps {
  contactId: string
  initialNotes: ContactNoteWithUser[]
...
export function NotesSection({
  ...
  const handleCreate = async () => {
    ...
    // Optimistic update
    const optimisticNote: ContactNoteWithUser = {
      id: `temp-${Date.now()}`,
    ...
    try {
      const result = await createNote(contactId, newNote.trim())
  ...
  const handleStartEdit = (note: ContactNoteWithUser) => {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const result = await updateNote(editingId, editContent.trim())
  ...
  const canModify = (note: ContactNoteWithUser) => {
    ...
    <div className="space-y-6">
      ...
      {notes.length === 0 ? (
        <div className="text-center py-8 text-muted-foreground">
          ...
          <p>Sin notas</p>
          <p className="text-sm">Agrega notas para recordar detalles importantes sobre este contacto.</p>
        </div>
      ) : (
        <Timeline>
