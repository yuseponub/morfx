...
3. **v4 comprehension sin fallback ante saturacion de Gemini (DIFERIDO — decision usuario 2026-05-28)** — La comprension de `somnio-sales-v4` usa UNICAMENTE `google('gemini-2.5-flash')` en `src/lib/agents/somnio-v4/comprehension.ts:86`, con los reintentos default del AI SDK (3 intentos, sin backoff custom) y SIN modelo de fallback. Cuando Google devuelve "This model is currently experiencing high demand" (pico de saturacion del proveedor — error transitorio, NO bug nuestro), los 3 intentos fallan y el turno muere con `AI_RetryError` → el bot no clasifica el mensaje y no responde (se ve como Turno con Intent="error"). Observado en sandbox el 2026-05-28. Hoy NO impacta produccion porque v4 esta DORMANT (0 workspaces). **Gatillo de activacion del fix:** cuando v4 este activo en prod y se observe este error en un turno real. **Fix planeado:** fallback a otro modelo (Anthropic Haiku o GPT-4o-mini) cuando Gemini retorne overload, y/o subir reintentos con backoff exponencial. Revisar tambien el sub-loop RAG (`src/lib/agents/somnio-v4/sub-loop/`) que tambien usa Gemini Flash. Decision del usuario: NO implementar ahora, hacerlo cuando falle en prod.
