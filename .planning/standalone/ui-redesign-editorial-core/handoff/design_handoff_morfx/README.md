...
## 4 · Módulos incluidos (9)

| # | Módulo | Archivo | Vistas/estados incluidos en el HTML | Vistas/estados faltantes (TODO Claude Code) |
|---|---|---|---|---|
| 0 | **Landing pública** | `mocks/landing.html` | Sitio marketing completo: header sticky, hero, secciones de producto, features, pricing, FAQ, footer. Carga `../colors_and_type.css`. | Variantes móviles detalladas, páginas secundarias (legal, blog, caso de estudio), modo dark si aplicara |
...
**Orden sugerido de implementación:** 0 (landing) → 1 → 2 → 3 → 4 → 5 → 6 → 7 → 8. La landing es independiente del resto de la app (distinto bundle, rutas públicas) y puede ir en paralelo.
...
└── mocks/
    ├── landing.html                ← sitio público de marketing
