/**
 * Somnio Recompra Agent — Comprehension Schema
 *
 * Zod schema for Claude Haiku structured output.
 * Single call per turn: intent + data extraction + classification.
 *
 * Fork of somnio-v3/comprehension-schema.ts.
 * Uses RECOMPRA_INTENTS (excludes contenido, formula, como_se_toma, efectividad).
 * Includes confirmar_direccion intent.
 */
...
export const MessageAnalysisSchema = z.object({
  intent: z.object({
    primary: z.enum(RECOMPRA_INTENTS),
    secondary: z.enum([...RECOMPRA_INTENTS, 'ninguno'] as const).describe(
  ...
  extracted_fields: z.object({
    ...
    entrega_oficina: z.boolean().nullable().describe(
      'true SOLO si senal CLARA de pickup en oficina: "oficina de inter", "recoger en oficina/sede", ' +
      '"no hay nomenclatura enviar a oficina", carrier usado COMO direccion sin calle real, ' +
    ...
    menciona_inter: z.boolean().nullable().describe(
      'true si menciona "inter"/"interrapidisimo" (o variantes) SIN senal clara de oficina/recoger/sede. ' +
      'Ej: "lo envian por interrapidisimo?", "interrapidisimo" suelto. ' +
      'NUNCA true si entrega_oficina es true — son mutuamente excluyentes. ' +
      'En caso de duda, preferir menciona_inter (preguntar es mas seguro).'
    ),
  }),
