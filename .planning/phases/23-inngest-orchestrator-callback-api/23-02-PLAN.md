...
**Error handling:** Wrap the dispatch step. If step.run throws (robot unreachable), the function fails. Since `retries: 0`, Inngest marks it as failed immediately. Add a `catch` in the outer function to mark the job as failed:

Actually, since `retries: 0`, any unhandled throw in a step.run means the function fails. The job status needs to be explicitly marked as failed. Use a try/catch wrapping the dispatch + wait section:

```typescript
try {
  // steps 2, 3, 4, 5
} catch (error) {
  // Mark job as failed on any unhandled error
  await step.run('mark-error-failed', async () => {
    await updateJobStatus(
      { workspaceId, source: 'inngest-orchestrator' },
      { jobId, status: 'failed' }
    )
