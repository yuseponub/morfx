...
Last activity: 2026-04-10 — Standalone envia-status-polling Plan 02 COMPLETE. Envia API client (native fetch, 10s timeout), carrier-events domain layer (insert/getLastCarrierEvent/getCarrierEventsByOrder), carrier-configs extended with status_polling_pipeline_id + status_polling_stage_ids, Inngest cron function (3-step: get-active-guides, poll-batch-N, process-changes) with batched polling (20/batch), change detection via cod_estado, feature flag ENVIA_AUTO_STAGE_MOVE (OFF). Cron: every 2h 5am-7pm Colombia. Registered in Inngest serve route. Commits: e39f9c8 + 3317981. Summary: .planning/standalone/envia-status-polling/02-SUMMARY.md. Plan 03 (DB migration + config UI) unblocked.
