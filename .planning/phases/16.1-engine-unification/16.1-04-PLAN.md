...
**CRITICAL for silent responses:** When ingest classifies message as 'datos', the agent returns `messages: ['[SANDBOX: Silent - clasificacion: datos, ...]']`. This message should be in the result. If the engine returns empty messages, the sandbox route should inject the classification debug message. Verify this path works correctly.
  </action>
  <verify>
