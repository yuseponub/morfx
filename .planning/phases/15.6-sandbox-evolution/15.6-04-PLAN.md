...
<task type="auto">
  ...
  <action>
**Verify ToggleGroup component is available:**

The `@radix-ui/react-toggle-group` package is already installed (v1.1.11 in package.json) and the shadcn component exists at `src/components/ui/toggle-group.tsx`. Verify the import resolves:
```bash
...
Similarly verify Slider is installed (should be from Plan 02):
```bash
