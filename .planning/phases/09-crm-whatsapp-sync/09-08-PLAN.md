...
must_haves:
  truths:
    - "Users can create context-specific tags (WhatsApp-only or Orders-only)"
    - "Tags scoped to WhatsApp only appear in WhatsApp tag selectors"
    - "Tags scoped to Orders only appear in Orders tag selectors"
    - "End-to-end CRM-WhatsApp sync works as verified by checkpoint"
  artifacts:
    ...
    - path: "src/app/(dashboard)/crm/pedidos/page.tsx"
      provides: "Orders page with scope-filtered tags"
      contains: "getTagsForScope"
  key_links:
    - from: "TagManager"
    ...
    - from: "OrdersPage"
      to: "getTagsForScope"
      via: "server call with orders scope"
      pattern: "getTagsForScope.*orders"
---
...
<task type="auto">
  <name>Task 2: Update Orders page to use scope-filtered tags</name>
  <files>src/app/(dashboard)/crm/pedidos/page.tsx</files>
  <action>
Update the Orders page to fetch only tags applicable to orders (scope 'orders' or 'both').

1. Change the import from getTags to getTagsForScope:
```typescript
import { getTagsForScope } from '@/app/actions/tags'
```

2. Update the Promise.all call to use getTagsForScope:
```typescript
const [orders, pipelines, products, contacts, tags] = await Promise.all([
  ...
  getActiveProducts(),
  getContacts(),
  getTagsForScope('orders')  // Changed from getTags()
])
```

This ensures that when users try to add tags to orders, they only see tags scoped to 'orders' or 'both' (not 'whatsapp'-only tags).
  </action>
  <verify>
```bash
cd /mnt/c/Users/Usuario/Proyectos/morfx-new && npx tsc --noEmit src/app/\\(dashboard\\)/crm/pedidos/page.tsx && grep -n "getTagsForScope" src/app/\\(dashboard\\)/crm/pedidos/page.tsx
```
  </verify>
  <done>
Orders page uses getTagsForScope('orders') to filter available tags by scope.
  </done>
</task>
...
7. Verify: "Solo WhatsApp" tag should NOT appear in the list (Task 2 ensures this works)
...
3. Orders page uses getTagsForScope('orders') - grep confirms pattern
4. All 6 functional tests pass
</verification>
