...
Phase: 42.1 (Observabilidad Bots Produccion) — IN PROGRESS (Wave 4 — Plan 07 COMPLETE)
Plan: 7/11
Status: Wave 4 Plan 07 COMPLETE. Flush + wire implementados. `src/lib/observability/flush.ts` creado con `flushCollector(collector)`: dedup de prompts via `resolvePromptVersions`, 1 INSERT en `agent_observability_turns` (UUID turnId), 3 children inserts paralelos chunked (CHUNK_SIZE=100) a `events`/`queries`/`ai_calls`, pino timing logs (`createModuleLogger('observability-flush')`) con info en cada flush + warn en counts >100 o duration >200ms + error con counts perdidos en fallo. **Anti-recursion (Pitfall 1):** usa `createRawAdminClient()` exclusivamente — `flush.ts` es el unico file en `src/lib/observability/` que lo importa. **Swallow-on-error (REGLA 6):** flush nunca throws hacia step.run, observabilidad jamas rompe un turno productivo. **collector.ts:** `flush()` ahora delega via dynamic import a `flushCollector(this)` para romper potencial ciclo y minimizar grafo estatico de quienes solo llaman record*. **agent-production.ts:** placeholder de Plan 05 reemplazado por wiring real con pattern `try { runWithCollector } catch { recordError + capture err } / step.run('observability-flush', flush) / if turnError throw` — el flush corre INCLUSO cuando run() throws (turnos fallidos son los mas interesantes), recordError corre ANTES del flush para que turn row lleve la causa, rethrow del turnError DESPUES del flush para preservar Inngest retries. Path `collector === null` (flag OFF) es byte-identical baseline (`if (!collector) return await run()`). Auto-fixes inline (Rule 1): `turnNumber` no existe en collector → insertar `null` literal, `queries.columns` es TEXT en schema (no array) → `JSON.stringify` cuando no es null, `prompt_version_id` non-null assertion → `?? null` defensivo. tsc clean en `src/lib/observability/*` y `src/inngest/functions/agent-production.ts`. `pnpm run build` local fallo unicamente por outage transitorio de Google Fonts en WSL (`Failed to fetch 'Geist'`) — error 100% ambiental, no relacionado con codigo. Wave 4 Plan 08 (cron purga + migration follow-up) desbloqueado.
Last activity: 2026-04-07 — Plan 07 ejecutado. Commits 3631d44 (flush.ts + collector.ts dynamic import) + 02919aa (agent-production.ts wiring).
