...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/lib/automations/trigger-emitter.ts"
      ...
      exports: ["emitOrderStageChanged", "emitTagAssigned", "emitTagRemoved", "emitContactCreated", "emitOrderCreated", "emitFieldChanged", "emitWhatsAppMessageReceived", "emitWhatsAppKeywordMatch", "emitTaskCompleted", "emitTaskOverdue"]
  key_links:
    - from: "src/lib/automations/action-executor.ts"
      to: "src/lib/tools/executor.ts"
      via: "Uses executeToolFromWebhook for WhatsApp actions (verified names: whatsapp.message.send, whatsapp.template.send)"
      pattern: "executeToolFromWebhook"
    - from: "src/lib/automations/action-executor.ts"
      to: "src/lib/tools/handlers/whatsapp/index.ts"
      via: "Tool names must match registered handler keys"
      pattern: "whatsapp\\.(message|template)\\.send"
...
**STEP 0: Verify tool names in registry (MANDATORY before implementation)**
Before implementing WhatsApp actions, read `@src/lib/tools/handlers/whatsapp/index.ts` and `@src/lib/tools/handlers/crm/index.ts` to confirm the exact tool names registered. The verified tool names as of planning time are:
- WhatsApp: `whatsapp.message.send`, `whatsapp.template.send`, `whatsapp.message.list`, `whatsapp.template.list`, `whatsapp.conversation.list`, `whatsapp.conversation.assign`, `whatsapp.conversation.close`
- CRM: `crm.contact.create`, `crm.contact.update`, `crm.contact.read`, `crm.contact.list`, `crm.contact.delete`, `crm.tag.add`, `crm.tag.remove`, `crm.order.create`, `crm.order.updateStatus`

If any tool name has changed, adapt the implementation accordingly. The action executor uses `executeToolFromWebhook` from `@/lib/tools/executor.ts` which requires `(toolName, inputs, workspaceId, requestMeta: { ip?, userAgent? })` — use `{ ip: 'inngest', userAgent: 'automation-engine' }` as requestMeta since this runs from Inngest.
...
3. **update_field**: Use `createAdminClient()` to update contact or order field. For custom_fields, merge into existing JSONB. After success, call `emitFieldChanged` with `cascadeDepth + 1` to support cascading field-change automations.
...
6. **send_whatsapp_template**: Use `executeToolFromWebhook` with tool name `whatsapp.template.send` (verified in registry). Resolve template variables from context. Requires contactId from trigger context (the tool handler looks up phone from contactId).

7. **send_whatsapp_text**: Use `executeToolFromWebhook` with tool name `whatsapp.message.send` (verified in registry). The handler expects `{ contactId, message }` as inputs. Resolve `{{variables}}` in text using `resolveVariables(action.params.text, context)`.

8. **send_whatsapp_media**: Use `executeToolFromWebhook` with tool name `whatsapp.message.send` (verified — same handler, no separate media handler exists). Send media URL as the message text or adapt the handler. NOTE: If WhatsApp media send requires a different handler signature, use `createAdminClient()` + direct 360dialog API call instead. Check `@src/lib/whatsapp/api.ts` for available send functions.
...
```
  ...
  <verify>All 11 action types have implementations. CRM actions use createAdminClient. WhatsApp actions use executeToolFromWebhook with VERIFIED tool names from registry (whatsapp.message.send, whatsapp.template.send). Grep for these names in src/lib/tools/handlers/whatsapp/index.ts to confirm they match. Cascade events emitted with depth+1 for state-modifying actions (tag, stage, order, field). TypeScript compiles.</verify>
